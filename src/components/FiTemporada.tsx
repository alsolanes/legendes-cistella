...
export function FiTemporada() {
  ...
  return (
    <div className="card resum-temporada" style={{ padding: 24 }}>
      <div style={{ fontSize: 48 }}>{pos === 1 ? '🏆' : pos <= 6 ? '🔥' : pos <= 10 ? '🛟' : '😔'}</div>
      ...
      <div style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
        <button className="btn btn-primari" onClick={reiniciar}>🔄 Nova temporada</button>
        <button className="btn btn-secundari" onClick={() => useJoc.getState().setPestanya('tauler')}>Tornar a la taula</button>
      </div>
    </div>
  );
}
